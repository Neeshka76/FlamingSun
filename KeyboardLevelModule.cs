...
namespace FlamingSun
{
    public class KeyboardLevelModule : LevelModule
    {
        ...
        public override void Update()
        {
            ...
            else
            {
                if (keyboard == null && Player.currentCreature != null)
                {
                    ...
                    handleKeyboard = Addressables.LoadAssetAsync<GameObject>("Neeshka.FlamingSun.Keyboard");
                ...
                {
                    currentStateKeyboard = (flamingSunController.data.FlamingSunTimeIdleFireballsButtonPressedGetSet
                                            ^ flamingSunController.data.FlamingSunTimeActivateFireballsButtonPressedGetSet
                                            ^ flamingSunController.data.FlamingSunTickThrowFireballsDelayButtonPressedGetSet
                                            ^ flamingSunController.data.FlamingSunNumFireballPerTickPressedGetSet
                                            ^ flamingSunController.data.FlamingSunChancesOfRandomThrowButtonPressedGetSet
                                            ^ flamingSunController.data.FlamingSunRadiusOfDetectionButtonPressedGetSet)
                                            && !keyboard.activeSelf;
                    // Rising edge
                    ...
                    if (flamingSunController.data.KeyboardFinishEnterButtonPressedGetSet == true && keyboard.activeSelf)
                    {
                        keyboard.SetActive(false);
                    }
                    // Set the new state
        ...
        public void ClickButtonEnter()
        {
            ...
            if (flamingSunController.data.ValueToAssignIsFloat == true)
            {
                if (float.TryParse(txtInput.text, out valueToAssignFloat))
                {
                    flamingSunController.data.ValueToAssignedFloatGetSet = valueToAssignFloat;
                    flamingSunController.data.KeyboardFinishEnterButtonPressedGetSet = true;
                    flamingSunController.data.ValueToAssignIsFloat = false;
                    txtInput.text = "";
                }
                else
                {
            ...
            if (flamingSunController.data.ValueToAssignIsInt == true)
            {
                if (int.TryParse(txtInput.text, out valueToAssignInt))
                {
                    flamingSunController.data.ValueToAssignedIntGetSet = valueToAssignInt;
                    flamingSunController.data.KeyboardFinishEnterButtonPressedGetSet = true;
                    flamingSunController.data.ValueToAssignIsInt = false;
                    txtInput.text = "";
                }
                else
                {
            ...
            if (flamingSunController.data.ValueToAssignIsUint == true)
            {
                if (uint.TryParse(txtInput.text, out valueToAssignUint))
                {
                    flamingSunController.data.ValueToAssignedUintGetSet = valueToAssignUint;
                    flamingSunController.data.KeyboardFinishEnterButtonPressedGetSet = true;
                    flamingSunController.data.ValueToAssignIsUint = false;
                    txtInput.text = "";
